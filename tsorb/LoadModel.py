...
from tsorb.utils.ApplianceProbabilityModifier import ApplianceProbabilityModifier
...
class AppliancesModel(object):
    def __init__(self, data_ex, occ_model, random_app_seed_per_run=True, pre_setup=False, get_hot_water=False):
        """
        ...
        self.pd_app_type_loads = pd.DataFrame(
            ...
            index=pd.date_range(start="00:00", periods=1440, freq="1min"),
        )
                        ...
                        if app.act_use_profile == "LEVEL":
                            ...
                            if active_occ > 0:
                                ...
                                pm = self.get_probability_modifier(
                                    activities, active_occ, app.act_use_profile, day_of_week
                                )
                                # Get the activity statistics
                                # for this profile at this time step
                                self._determine_starting_event(minute, app, pm.get_modifiers[ten_minute_count])
                    else:
    ...
    def get_probability_modifier(self, activities, active_occ, app_use_profile, day_of_week):
        return [
            x
            for x in activities
...
class LightingModel(object):
    ...
    def run_model(self, year, day_of_week, day_in_year):
        ...
        irr_threshold = Calculations.calc_from_normal_distr(self._mean_irradiance, self._sd_irradiance)
        if self.weather_data is None:
            ...
            irradiance_data = self._irradiance[
                (self._irradiance.index.dayofyear == day_in_year) & (self._irradiance.index.year == year)
            ].values
        if not irradiance_data.size:
            raise ValueError(
                "No irradiance data available for year " + str(year) + " and day in year " + str(day_in_year)
            )
        ...
        for bulb in self.loads:
            ...
            while minute < 1440:
                ...
                irr_below_threshold = irradiance < irr_threshold or np.random.random() < 0.05
                ...
                if irr_below_threshold and (np.random.random() < sharing_and_weight_factor):
    ...
    def _get_bulbs(self):
        ...
        bulb_rating = np.concatenate(
            (p_glueh_random, p_spar_random, p_halogen_random, p_led_random, p_leucht_random), axis=0
        )
